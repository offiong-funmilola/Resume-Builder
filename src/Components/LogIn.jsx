...
function LogIn() {
    const { passwordhandler, confirmHandler, type, retrieveUserCvs, login, makePostRequest, REGEX_PASSWORD } = useContext(ResumeContext)
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            let response = await makePostRequest(`/api/v1/login`, values)
            if (response.user) {
                retrieveUserCvs()
                login(response.user)
            }
    ...
    return (
        <div className='w-full h-screen flex flex-col justify-center items-center bg-white'>
            <div className='w-full md:w-1/2 h-3/4 bg-gray-100 flex items-center justify-center'>
                <form onSubmit={formik.handleSubmit} className='w-full h-3/4 bg-inherit flex flex-col items-center justify-center'>
                    ...
                    <div className='w-3/4 mb-3 relative'>
                        ...
                        {type === 'password' ? <FaRegEyeSlash onClick={passwordhandler} className='absolute top-1 translate-y-2/3 right-4 text-xl'/> : <FaRegEye onClick={confirmHandler} className='absolute top-0 translate-y-2/3 right-3 text-xl'/>}
